...
export default function BookingTablePage() {
	...
	const handleSubmit = async (values: Partial<Record<string, any>>) => {
		...
		if (data.data.payDepositNow) {
			router.push(
				`/restaurants/${slug}/booking/checkout/${data.data.booking._id}`,
			);
		} else {
			router.push(
				`/restaurants/${slug}/booking/success/${data.data.booking._id}`,
			);
		}
	};
			...
			<div className="col-span-2 space-y-6">
				<CustomCard>
					...
					<p className="text-sm text-gray-500">
						Vui lòng điền thông tin bên dưới để chúng tôi có thể
						liên hệ bạn.
					</p>
							...
							<div className="mt-3 space-y-2 text-sm">
								<div className="flex justify-between">
									...
									<span className="font-semibold">
										{previewData.basePrice?.toLocaleString() ??
											0}
										đ
									</span>
								</div>
								...
								<div className="mt-2 flex justify-between border-t pt-2">
									...
									<span className="font-bold">
										{previewData.finalPrice?.toLocaleString() ??
											0}
										đ
									</span>
								</div>
