...
import { useGetTables, useCreateTable, useUpdateTablePosition } from '../hooks/useTableCrud';
...
function findEmptyPosition(existingTables: ITable[]): { x: number; y: number } {
    ...
    for (let row = 0; ; row++) {
        for (let col = 0; col < Math.floor((CANVAS_WIDTH - padding * 2) / GRID_STEP) + 1; col++) {
...
function TableManagementPage() {
    ...
    const [deleteAreaTarget, setDeleteAreaTarget] = useState<IArea | null>(null);
    const [doubleClickAreaId, setDoubleClickAreaId] = useState<string | null>(null);
    ...
    const handlePositionChange = (tableId: string, x: number, y: number) => {
        updateTablePosition.mutate({ tableId, payload: { x, y } });
    };
        ...
        <div className="flex h-full flex-col gap-4">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2">
                    ...
                    >
                        {editMode ? 'Đang chỉnh sửa' : 'Chỉnh sửa layout'}
                    </Button>
                    <Button
            ...
            <div className="flex-1 overflow-auto">
                {areasLoading || tablesLoading ? (
                    // Skeleton canvas
                    <div
                        className="animate-pulse overflow-hidden rounded-2xl border border-slate-200 bg-[#fafaf8]"
            ...
            <AreaFormModal
                ...
                onValuesChange={(v) =>
                    areaModal.setSelectedRecord((prev) => ({ ...prev, ...v }) as IArea)
                }
