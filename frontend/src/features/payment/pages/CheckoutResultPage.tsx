...
const getBookingId = (payment: any, queryBookingId: string) => {
	const rawBookingId =
		payment?.booking?._id ?? payment?.bookingId._id ?? queryBookingId ?? "";
...
function CheckoutResultPage({ id }: CheckoutResultProps) {
	...
	const successLink =
		isPaymentSuccess && slug && bookingId
			? `/restaurants/${slug}/booking/success/${bookingId}`
			: "";
	const retryLink =
		!isPaymentSuccess && slug && bookingId
			? `/restaurants/${slug}/booking/checkout/${bookingId}`
			: "";
			...
			<div className="flex h-full items-center justify-center">
				<CustomCard className="max-w-3xl mx-auto p-6 text-center">
					<h2 className="text-xl font-semibold">
						Lỗi tải kết quả thanh toán
					</h2>
					<p className="mt-2 text-sm text-gray-500">
						{String(
							error?.message ??
								"Đã xảy ra lỗi khi lấy thông tin thanh toán.",
						)}
					</p>
					<div className="mt-6">
						<Button
			...
			<CustomCard>
				<div className="space-y-4">
					...
					<p className="text-sm text-gray-500">
						Thanh toán được lấy từ ID giao dịch:{" "}
						<strong>{id}</strong>
					</p>
				</div>
				...
				<div className="mt-6 grid gap-4 sm:grid-cols-2">
					<InfoValueCard
						label="Booking ID"
						value={bookingId || "Không có thông tin"}
					/>
